...
import { prepareData, data } from "../firebase/intentions";
import DialogFallback from "../root/DialogFallback";
import { isAuthorizedFunc, isLogged, isReady } from "../firebase/auth";

const ElementWrapper = (props: FlowProps) => <div class="mx-auto min-w-full max-w-full sm:max-w-2xl md:max-w-3xl lg:max-w-4xl px-2 sm:px-0 py-4 sm:min-w-96">{props.children}</div>
...
export default function Uczestnicy() {
    ...
    createEffect(async () => {
        ...
        console.log("Element do pokazania:", showElement())
                ...
                <div>
                    <Switch>
                        <Match when={showElement() === "table"}>
                            <ElementWrapper>
                                <Suspense fallback={<DialogFallback notMinHeight />}>

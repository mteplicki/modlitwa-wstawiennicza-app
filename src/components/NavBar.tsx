...
import { Accessor, Show, VoidProps, createSignal } from "solid-js"
import { navigate } from "../router/MyRouter"
            ...
            <nav class="max-w-[85rem] w-full mx-auto px-4 flex flex-wrap basis-full items-center justify-between" aria-label="Global">
                ...
                <div class="md:order-3 flex justify-end gap-x-2 grow basis-0">
                    <Show when={!isInStandaloneMode()}>
                        <button type="button" class="transition-all md:hidden hs-collapse-toggle p-2.5 rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 dark:bg-transparent dark:border-gray-700 dark:text-white dark:hover:bg-white/10" onclick={() => setExpanded((prev) => !prev)} aria-controls="navbar-alignment1" aria-label="Toggle navigation">
                            ...
                            <div class="hamburger w-5 h-5" id="hamburger-1" classList={{ "is-active": expanded() }}>
                                <span class="line bg-gray-800 dark:bg-white"></span>
                                <span class="line bg-gray-800 dark:bg-white"></span>
                                <span class="line bg-gray-800 dark:bg-white"></span>
                            </div>
                        </button>
                ...
                <Show when={!isInStandaloneMode()}>
                    <div ref={ref} style={{ "max-height": ((expanded()) ? ref?.scrollHeight?.toString() + "px" : 0) }} id="navbar-alignment1" class={`overflow-hidden transition-all duration-300 basis-full md:hidden grow`}>
                    ...
                    <div id="navbar-alignment2" class={`hidden overflow-hidden transition-all duration-300 basis-full grow md:grow-0 md:basis-auto md:block md:order-2 md:mx-5`}>
                        <div class="flex flex-col gap-5 mt-5 md:flex-row md:items-center md:mt-0 md:ps-0">
                            <button type="button" class={`${props.selected() === "/intencje" ? "text-tertiary" : "text-white"} transition font-medium hover:text-lime-600 cursor-pointer`} onclick={() => navigate("/intencje")}>Wyślij intencje</button>
                            <button type="button" class={`${props.selected() === "/uczestnicy" ? "text-tertiary" : "text-white"} transition font-medium  hover:text-lime-600 cursor-pointer`} onclick={() => navigate("/uczestnicy")}>Panel uczestnika</button>
                        </div>

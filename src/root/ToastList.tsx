    ...
    return (
        <div ref={(ref) => { wrapper.resolveFunc(ref) }} class="w-full overflow-y-hidden z-50 sm:w-96 bg-white border border-gray-200 rounded-xl shadow-lg dark:bg-gray-800 dark:border-gray-700" role="alert">
            <div class="flex p-4">
                ...
                <div class="ms-4">
                    <h3 class="text-gray-800 font-semibold dark:text-white text-md">
                        {toast.title}
                    </h3>
                    <div class="mt-1 text-md text-gray-600 dark:text-gray-400">
                        {toast.description}
                    </div>
                    <div class="mt-4">
                        <div class="flex space-x-3">
                            <Show when={toast.action1}>
                                <button onclick={toast.action1?.action} type="button" class="inline-flex items-center gap-x-2 text-md font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:text-blue-800 dark:text-blue-500 dark:focus:text-blue-400">
                                    {toast.action1?.title}
                                </button>
                            </Show>
                            <Show when={toast.action2}>
                                <button onclick={toast.action2?.action} type="button" class="inline-flex items-center gap-x-2 text-md font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 disabled:opacity-50 disabled:pointer-events-none focus:outline-none focus:text-blue-800 dark:text-blue-500 dark:focus:text-blue-400">
                                    {toast.action2?.title}
                                </button>
                            </Show>
                        </div>
...
export function ErrorToast({ wrapper }: VoidProps<{ wrapper: ToastWrapper }>) {
    const { toast } = wrapper
    return <div ref={(ref) => { wrapper.resolveFunc(ref) }} class="z-50 w-full sm:w-96 overflow-y-hidden bg-white border border-gray-200 rounded-xl shadow-lg dark:bg-gray-800 dark:border-gray-700" role="alert">
        <div class="flex p-4">
            <div class="flex-shrink-0">
            ...
            <For each={toasts()}>
                {(item) =>
                    <Switch >
                        ...
                        <Match when={item.toast.type === "error"}>
                            <ErrorToast wrapper={item} />
                        </Match>
                    </Switch>
                }
            </For>
